<?php
    ...
    if(isset($_GET['c_id'])){
        $id_conversa = base64_decode($_GET['c_id']);
        $query = mysqli_query($link, "SELECT * FROM conversas WHERE id_conversa='$id_conversa'");
        if(mysqli_num_rows($query) > 0){
            while ($m = mysqli_fetch_assoc($query)) {
                $usuario_envio = $m['usuario_envio'];
                $usuario_destino = $m['usuario_destino'];
                $message = $m['message'];
                $user = mysqli_query($link, "SELECT nome FROM usuario WHERE id_usuario='$usuario_envio'");
